...
(function(){
  ...
  function iconFor(name,type){
    var s=String(name||'');
    if(/แม่|วันแม่|เฉลิมพระชนมพรรษา.*ราชินี|ราชินี/.test(s))return 'crown';
    if(/พรรษา|มาฆ|วิสาข|อาสาฬห|บูชา|วันพระ/.test(s))return 'lotus';
    if(/รพี|กฎหมาย|ยุติธรรม|ศาล/.test(s))return 'scales';
    if(/สตรี|สงกรานต์|พืชมงคล|ดอกไม้|ชมพู/.test(s))return 'flower';
    if(/ชาติ|รัฐธรรมนูญ|จักรี|แรงงาน|ฉัตรมงคล|สื่อสาร|สัตวแพทย์/.test(s))return 'flag';
  ...
  function renderUpcoming(){
    ...
    box.innerHTML=show.map(function(k){
      var item=state().holidays[k]||{},t=item.type||'official',icon=iconFor(item.name,t);
      return '<div data-key="'+k+'" class="h32-up-item">'
        +'<span class="h32-event-icon icon-'+icon+' '+t+'" aria-hidden="true"></span>'
        +'<span class="h32-event-name"><b>'+esc(item.name||'วันหยุด')+'</b></span>'
        +'<span class="h32-event-date">'+formatThaiDate(k)+'</span>'
        +'<span class="h32-type-badge '+t+'">'+typeLabel(t)+'</span>'
        +'<button type="button" class="h32-edit-event" data-edit="'+k+'">แก้ไข</button>'
        +'</div>';
    }).join('');
