/* HISTORY V33.3 — full reference layout + working functions */
(function(){
  'use strict';
  ...
  function workflowStatus(d){
    if(d&&d.workflow&&d.workflow.locked)return 'locked';
    return d&&d.workflow&&d.workflow.status||'draft';
  }
  function unitName(){var u=B.getUnit?B.getUnit():{};return u.name||'หน่วยงานหลัก'}
  function schedulerName(d){
    var a=d&&d.workflow&&Array.isArray(d.workflow.audit)?d.workflow.audit:[];
    for(var i=0;i<a.length;i++)if(a[i].user)return a[i].user;
    var u=B.getUnit?B.getUnit():{};return u.scheduler||'ผู้จัดตารางเวร';
  }
  function findAudit(d,words){
    var a=d&&d.workflow&&Array.isArray(d.workflow.audit)?d.workflow.audit:[];
    for(var i=0;i<a.length;i++){
      var t=String(a[i].action||'');
      if(words.some(function(w){return t.indexOf(w)>=0}))return a[i];
    }
    return null;
  }
  function noteFor(d){
    var s=workflowStatus(d);
    if(s==='locked')return 'ตารางประจำเดือน';
    if(s==='approved')return 'ปรับเปลี่ยนตามคำขอ';
    if(s==='submitted')return 'รออนุมัติ';
    return 'ร่างแรก';
  }
  ...
  function shell(){
    ...
    '<div class="h33-shell">'+
      '<header class="h33-page-head">'+
        ...
        '</div>'+
        '<span class="h33-version">V33.3 • History</span>'+
      '</header>'+
  ...
  function buildRecords(){
    ...
    years.forEach(function(y){
      ...
      arr.forEach(function(r){
        ...
        records.push({
          year:y,month:r.month,data:d,status:workflowStatus(d),unit:unitName(),scheduler:schedulerName(d),
          savedAt:sv&&sv.at||d.savedAt||'',approvedAt:ap&&ap.at||'',lockedAt:lk&&lk.at||'',note:noteFor(d)
        });
      });
    });
  ...
  function setupFilters(){
    ...
    if(ys){
      var years=B.getAvailableYears?B.getAvailableYears():[];
      ys.innerHTML='<option value="all">ทั้งหมด</option>'+years.map(function(y){return '<option value="'+y+'">'+y+'</option>'}).join('');
    }
  ...
  function applyFilters(){
    ...
    filtered=records.filter(function(r){
      ...
      if(q){
        var hay=[MONTHS[r.month],r.year,r.unit,r.scheduler,r.note,statusLabel(r.status)].join(' ').toLowerCase();
  ...
  function renderStats(){
    ...
    var approved=filtered.filter(function(r){return r.status==='approved'||r.status==='locked'}).length;
    var locked=filtered.filter(function(r){return r.status==='locked'}).length;
    var last=filtered[0];
  ...
  function renderRows(){
    ...
    body.innerHTML=slice.length?slice.map(function(r){
      return '<tr>'+
        '<td><div class="h33-month-cell"><span class="h33-month-ico">🗓️</span><span>'+esc(MONTHS[r.month])+' '+r.year+'</span></div></td>'+
        '<td>'+esc(r.unit)+'</td>'+
        '<td><span class="h33-status '+r.status+'">'+statusIcon(r.status)+' '+statusLabel(r.status)+'</span></td>'+
        '<td><div class="h33-user"><span class="h33-avatar">👩🏻</span><span>'+esc(r.scheduler)+'</span></div></td>'+
        '<td>'+fmt(r.savedAt)+'</td><td>'+fmt(r.approvedAt)+'</td><td>'+fmt(r.lockedAt)+'</td>'+
        '<td>'+esc(r.note)+'</td>'+
        '<td><div class="h33-row-actions"><button class="h33-open" data-open="'+r.year+'|'+r.month+'">◉ เปิดดู</button><button class="h33-more" data-more="'+r.year+'|'+r.month+'">⋮</button></div></td>'+
      '</tr>';
    }).join(''):'<tr><td colspan="9" style="text-align:center;padding:34px;color:#98a1af">ไม่พบประวัติการจัดเวรตามตัวกรอง</td></tr>';
  ...
  function renderPager(){
    ...
    for(var p=1;p<=pages;p++){
      if(p<=5||p===pages||Math.abs(p-page)<=1)html+='<button data-p="'+p+'" class="'+(p===page?'active':'')+'">'+p+'</button>';
      else if(p===6&&pages>7)html+='<span>…</span>';
    }
    html+='<button data-p="next" '+(page>=pages?'disabled':'')+'>›</button>';
    host.innerHTML=html;
    host.querySelectorAll('button').forEach(function(b){
      b.onclick=function(){
        var v=this.dataset.p;
  ...
  function addRecent(y,m){
    var a=readRecent().filter(function(x){return !(x.year===y&&x.month===m)});
    a.unshift({year:y,month:m,at:new Date().toISOString()});
    try{localStorage.setItem(recentKey(),JSON.stringify(a.slice(0,30)))}catch(e){}
    renderRecent();
  }
  function openRecord(y,m,type){if(B.openMonth)B.openMonth(y,m,type);addRecent(y,m)}

  function renderRecent(){
    var host=$('historyRecentV24');if(!host)return;
    var a=readRecent(),show=recentAll?a:a.slice(0,5);
    host.innerHTML=show.length?show.map(function(x){
      var r=records.find(function(z){return z.year===x.year&&z.month===x.month});
      return '<div class="h33-recent-item"><span class="h33-list-icon">🗓️</span><span class="h33-list-main"><b>'+esc(MONTHS[x.month])+' '+x.year+'</b><small>'+esc(r?r.unit:unitName())+'</small></span><span class="h33-list-time">'+fmt(x.at)+' ›</span></div>';
    }).join(''):'<div style="padding:22px;text-align:center;color:#9aa3b1">ยังไม่มีรายการที่เข้าดูล่าสุด</div>';
  }

  function allAudit(){
    var out=[];
    records.forEach(function(r){
      var a=r.data&&r.data.workflow&&Array.isArray(r.data.workflow.audit)?r.data.workflow.audit:[];
      a.forEach(function(x){out.push({x:x,r:r})});
    });
    out.sort(function(a,b){return String(b.x.at||'').localeCompare(String(a.x.at||''))});
    return out;
  }

  function auditStyle(action){
    var t=String(action||'');
    if(t.indexOf('ล็อก')>=0)return ['🔒','ล็อกตารางเวร','#8660d3'];
    if(t.indexOf('อนุมัติ')>=0)return ['✅','อนุมัติตารางเวร','#31aa80'];
    if(t.indexOf('ส่ง')>=0)return ['✈️','ส่งตรวจตารางเวร','#4a9fe0'];
    if(t.indexOf('แก้')>=0)return ['✏️','แก้ไขตารางเวร','#ef6597'];
    return ['📝',t||'บันทึกร่างตารางเวร','#e3a12d'];
  }

  function renderAudit(){
    var host=$('historyAuditV24');if(!host)return;
    var a=allAudit(),show=auditAll?a:a.slice(0,5);
    host.innerHTML=show.length?show.map(function(z){
      var st=auditStyle(z.x.action);
      return '<div class="h33-audit-item"><span class="h33-list-icon" style="background:'+st[2]+'16;color:'+st[2]+'">'+st[0]+'</span><span class="h33-list-main"><b style="color:'+st[2]+'">'+esc(st[1])+'</b><small>'+esc(MONTHS[z.r.month])+' '+z.r.year+' ('+esc(z.r.unit)+') · '+esc(z.x.user||z.r.scheduler)+'</small></span><span class="h33-list-time">'+fmt(z.x.at)+'</span></div>';
    }).join(''):'<div style="padding:22px;text-align:center;color:#9aa3b1">ยังไม่มี Audit Log</div>';
  }

  function renderAll(){
    renderStats();renderRows();renderRecent();renderAudit();
    if($('historyInfo'))$('historyInfo').textContent='พบ '+filtered.length+' รายการ';
  }

  function exportCsv(){
    var rows=[['เดือน/ปี','หน่วยงาน','สถานะ','ผู้จัดเวร','วันที่บันทึก','วันที่อนุมัติ','วันที่ล็อก','หมายเหตุ']];
    filtered.forEach(function(r){rows.push([MONTHS[r.month]+' '+r.year,r.unit,statusLabel(r.status),r.scheduler,fmt(r.savedAt),fmt(r.approvedAt),fmt(r.lockedAt),r.note])});
  ...
  function resetFilters(){
    if($('historySearchV24'))$('historySearchV24').value='';
    if($('historyMonthV33'))$('historyMonthV33').value='all';
    if($('historyYearSelectV33'))$('historyYearSelectV33').value='all';
    if($('historyStatusV24'))$('historyStatusV24').value='all';
    if($('historyUnitV33'))$('historyUnitV33').value='all';
    applyFilters();
  }
